...
def main():
    ...
    assert_ret("10", "{ i=0; while(i<10) { i=i+1; } return i; }")

    assert_ret("3", "{ x=3; return *&x; }")
    assert_ret("3", "{ x=3; y=&x; z=&y; return **z; }")
    assert_ret("5", "{ x=3; y=5; return *(&x+1); }")
    assert_ret("3", "{ x=3; y=5; return *(&y-1); }")
    assert_ret("5", "{ x=3; y=5; return *(&x-(-1)); }")
    assert_ret("5", "{ x=3; y=&x; *y=5; return x; }")
    assert_ret("7", "{ x=3; y=5; *(&x+1)=7; return y; }")
    assert_ret("7", "{ x=3; y=5; *(&y-2+1)=7; return x; }")
    assert_ret("5", "{ x=3; return (&x+2)-&x+3; }")
