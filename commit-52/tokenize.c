...
static int read_escaped_char(char **new_pos, char *p) {
  ...
  *new_pos = p + 1;
  ...
  // For more info, read "Reflections and Trusting Trust" by Ken Thompson.
  // https://github.com/rui314/chibicc/wiki/thompson.pdf
  switch(*p) {
    case 'a': return '\a';
    case 'b': return '\b';
    case 't': return '\t';
...
// Initialize line infor for all tokens
...
static char *read_file(char *path) {
  ...
  if(strcmp(path, "-") == 0) {
    // By convention, read it from stdin if a given filename is "-".
    fp = stdin;
  } else {
    fp = fopen(path, "r");
    if(!fp)
