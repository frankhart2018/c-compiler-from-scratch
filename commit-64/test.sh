...
assert 8 'int main() { return add2(3, 5); } int add2(int x, int y) { return x+y; }'
assert 2 'int main() { return sub2(5, 3); } int sub2(int x, int y) { return x-y; }'
...
assert 720 'int main() { return fact(6); } int fact(int x) { if (x == 0) return 1; return x * fact(x - 1); }'
...
assert 1 'int main() { return sub_long(7, 3, 3); } int sub_long(long a, long b, long c) { return a - b - c; }'
assert 16 'int main() { struct { char a; long b; } x; return sizeof(x); }'
assert 8 'int main() { long x; return sizeof(x); }'

assert 1 'int main() { return sub_short(7, 3, 3); } int sub_short(short a, short b, short c) { return a - b - c; }' 
...
assert 12 'int main() { char (x[3])[4]; sizeof(x); }'
assert 4 'int main() { char (x[3])[4]; sizeof(x[0]); }'
assert 3 'int main() { char *x[3]; char y; x[0]=&y; y=3; x[0][0]; }'
assert 4 'int main() { char x[3]; char (*y)[3]=x; y[0][0]=4; y[0][0]; }'
...
assert 16 'typedef int MyInt2[4]; int main() { MyInt2 x; sizeof(x); }'
